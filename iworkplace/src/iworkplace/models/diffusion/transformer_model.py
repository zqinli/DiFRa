...
#         t_emb_input = timestep_embedding(timesteps, self.hidden_t_dim).to(weight_dtype)
...
#         combined_embeds = torch.cat([time_token, emb_x], dim=1)

#         new_seq_length = combined_embeds.size(1) 
#         position_ids = self.position_ids[:, : new_seq_length ]
#         pos_emb = self.position_embeddings(position_ids)

#         combined_with_pos_fp32 = combined_embeds.float() + pos_emb.float()
#         combined_with_pos = combined_with_pos_fp32.to(weight_dtype)
...
#             print("Warning: 'input_attention_mask' not found in TransformerNetModel kwargs. Defaulting to all ones.")
#             original_mask = torch.ones(emb_x.shape[0], emb_x.shape[1], 
#                                     device=emb_x.device, dtype=torch.long)

#         time_token_mask = torch.ones(emb_x.shape[0], 1, 
#                                     device=emb_x.device, dtype=torch.long)
...
# 显式导入 BertEncoder 虽然不一定直接实例化，但有助于类型检查或理解
from transformers.models.bert.modeling_bert import BertEncoder 
import numpy as np
...
class TransformerNetModel(nn.Module):
    def __init__(self,
        ...
        self.hidden_t_dim = hidden_t_dim
        self.output_dims = output_dims
        self.dropout_prob = dropout # 重命名以防混淆
        self.logits_mode = logits_mode
        self.hidden_size = config.hidden_size
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        # 0. 准备类型和 Time Embedding
        try:
            # 尝试获取 time_embed 层的 dtype，以防 float16/bfloat16 混合精度训练时出错
            weight_dtype = next(self.time_embed.parameters()).dtype
        except StopIteration:
            weight_dtype = torch.float32
        ...
        emb_t = self.time_embed(t_emb_input) 
        time_token = emb_t.unsqueeze(1) # [Batch, 1, Hidden]
        ...
        # 2. 为数据部分单独计算位置编码 (从 0 到 seq_len-1)
        seq_length = emb_x.size(1)
        
        # 确保 position_ids 在正确的设备上
